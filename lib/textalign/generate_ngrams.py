...
class Ngrams:
    ...
    def generate(self, file_path, output_path, is_philo_db=False, db_path=None, metadata=None, workers=4, ram="50%"):
        ...
        os.system(
            r"""for i in {}/temp/*; do cat $i; done | sort -T {} -S {}% | uniq -c | sort -rn -T {} -S {}% | awk '{{print $2"\t"$3}}' |
                tee {}/index/index.tab | awk '{{print $2}}' > {}/index/most_common_ngrams.txt""".format(
                output_path, output_path, mem_usage, output_path, mem_usage, output_path, output_path
            )
        )
    ...
    def process_file(self, input_file):
        ...
        doc_ngrams = []
        metadata = {}
        if self.config["stemmer"] is True:
        ...
        with open(input_file) as file:
            ...
            for word_obj in doc:
                ...
                word = preprocessor.normalize(word, stemmer)
                ...
                if len(ngram_obj) == self.config["window"]:  # window is ngram+gap
                    ...
                    for value in iterator:
                        ...
                        if self.config["word_order"] is True:
                            ...
                            current_ngram = sorted(
                                current_ngram_list
                            )  # we sort ngram by word so as to make word order irrelevant
                            current_ngram = "_".join(current_ngram_list)
                    ...
                    ngram_obj.popleft()
        if (
            self.config["text_object_level"] == "doc" and current_text_id is not None
        ):  # make sure the file is not empty (no lines so never entered loop)
            if self.debug:
                self.__write_to_disk(debug_ngrams, current_text_id)
            if self.metadata_done is False:
                metadata[current_text_id] = self.__get_metadata(current_text_id)
...
def parse_command_line():
    ...
    optional.add_argument("--config", help="configuration file used to override defaults", type=str, default="")
    optional.add_argument("--cores", help="number of cores used for parsing and generating ngrams", type=int, default=4)
    required.add_argument("--file_path", help="path to files", type=str)
    optional.add_argument(
        "--lemmatizer", help="path to a file where each line contains a token/lemma pair separated by a tab "
    )
    optional.add_argument(
        "--mem_usage",
        help="how much max RAM to use: expressed in percentage, no higher than 90%%",
        type=str,
        default="80%%",
    )
    optional.add_argument(
        "--is_philo_db", help="define is files are from a PhiloLogic4 instance", action="store_true", default=False
    )
    optional.add_argument("--metadata", help="metadata for input files", default=None)
    optional.add_argument("--text_object_level", help="type of object to split up docs in", type=str, default="doc")
    optional.add_argument("--output_path", help="output path of ngrams", type=str, default="./output")
...
if __name__ == "__main__":
    ...
    NGRAM_GENERATOR = Ngrams(
        stopwords=ARGS["stopwords"],
        lemmatizer=ARGS["lemmatizer"],
        text_object_level=ARGS["text_object_level"],
        gap=ARGS["gap"],
    )
    NGRAM_GENERATOR.generate(
        ARGS["file_path"],
        ARGS["output_path"],
