...
| `test_banality_parallel.py` | parallel banality detection emits byte for byte what the serial pass emits, over several frame layouts and worker counts, including more workers than frames and records that already carry the field. Synthesises its own n-gram index and frequent-key file, so it needs no corpus |
